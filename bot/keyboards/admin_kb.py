...
from database.models import (
    User, TelegramGroup, ParserAccount, Proxy, Category,
)
...
def categories_list_kb(categories: list[Category]) -> InlineKeyboardMarkup:
    ...
    for cat in categories:
        ...
        owner_tag = f" · u{cat.owner_id}"
        builder.row(
            InlineKeyboardButton(
                text=f"{status}  {cat.name}{owner_tag}",
                callback_data=f"adm:cat:detail:{cat.id}",
                style=style,
            )
        )
    builder.row(InlineKeyboardButton(text="➕ Добавить категорию", callback_data="adm:cat:create", style="success"))
    builder.row(InlineKeyboardButton(text="◀ Назад", callback_data="adm:main", style="primary"))
    return builder.as_markup()
