...
from bot.keyboards.admin_kb import categories_list_kb, category_detail_kb, category_accounts_kb, cancel_kb
from bot.states import CategorySG
from database.models import Category, User, CategoryAccount, ParserAccount
...
def _cat_detail_text(cat: Category) -> str:
    ...
    return (
        f"📂 <b>{cat.name}</b> · u{cat.owner_id} — {status}\n"
        "━━━━━━━━━━━━━━━━━━━━━\n\n"
...
async def process_cat_name(message: Message, state: FSMContext, session: AsyncSession) -> None:
    cat = Category(owner_id=message.from_user.id, name=message.text.strip())
    session.add(cat)
...
async def cb_cat_addkw(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.edit_text(
        ...
        "  порядок и расстояние не важны)\n"
        "• Разные строки — это синонимы (OR)\n\n"
        "<i>Пример:\n"
        "<code>ищу смм специалиста</code> — сработает на «ищу хорошего смм специалиста»\n"
